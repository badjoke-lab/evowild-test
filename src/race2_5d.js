...
Promise.all(Object.keys(MORPHS).map(loadImage))
  .then(() => {
    stage.dataset.assets = "ready";
    assetState.textContent = "S / P / E / A race assets ready";
  })
  .catch((error) => {
...
function makeRacers() {
  return names.map((name, index) => {
    const morph = index === selectedId - 1 ? "A" : morphCycle[index % morphCycle.length];
...
function drawRacer(racer, cameraDistance, ppm, focusX) {
  ...
  const bob = Math.abs(Math.sin(stridePhase)) * stat.bob * scale;
  const tilt = Math.sin(stridePhase * 0.5) * 0.018 * pace;
