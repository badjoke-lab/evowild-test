...
const SHAPE_REVIEW = params.get("shape");
const SHAPE_REVIEW_V2 = SHAPE_REVIEW === "v2";
const SHAPE_REVIEW_V21 = SHAPE_REVIEW === "v21";
  ...
  inspect: {
    id: SHAPE_REVIEW_V21
      ...
      : SHAPE_REVIEW_V2
        ...
        : "hunyuan-s-lod2",
    url: SHAPE_REVIEW_V21
      ? "/evowild-test/models/evowild-s/source-lod2-shape-v21.glb"
      : SHAPE_REVIEW_V2
        ? "/evowild-test/models/evowild-s/source-lod2-shape-v2.glb"
        : "/evowild-test/models/evowild-s/source-lod2.glb",
    targetHeight: 2.95,
    animated: false
  },
  focus: {
...
function makePowerLimb(parent, upperMat, lowerMat, jointMat, plateMat, side, fore) {
  const hip = new THREE.Group();
  hip.position.set(
    side * (fore ? 0.50 : 0.47),
    fore ? -0.14 : -0.12,
  ...
  const upperLen = fore ? 0.68 : 0.72;
  const lowerLen = fore ? 0.56 : 0.60;
  const cannonLen = fore ? 0.26 : 0.28;

  const upper = makeMesh(
    new THREE.CylinderGeometry(0.17, 0.235, upperLen, 6),
    upperMat,
    hip,
    [0, -upperLen / 2, 0.05]
  );
  upper.rotation.z = side * (fore ? 0.055 : 0.065);
  ...
  makeMesh(
    new THREE.IcosahedronGeometry(fore ? 0.16 : 0.175, 1),
    jointMat,
    knee
  );

  const lower = makeMesh(
    new THREE.CylinderGeometry(0.11, 0.16, lowerLen, 6),
    lowerMat,
    knee,
    [0, -lowerLen / 2, 0.05]
  );
  lower.rotation.z = side * (fore ? -0.03 : 0.035);

  const ankle = new THREE.Group();
  ankle.position.set(0, -lowerLen, 0.10);
  knee.add(ankle);
  ...
  const cannon = makeMesh(
    new THREE.CylinderGeometry(0.080, 0.115, cannonLen, 6),
    upperMat,
    ankle,
    [0, -cannonLen / 2, 0.04]
  );
  ...
  const sole = makeMesh(
    new THREE.BoxGeometry(0.28, 0.11, 0.40),
    jointMat,
    foot,
    [0, -0.025, 0.11]
  );
  ...
  [-1, 1].forEach((toeSide) => {
    const toe = makeTaperedPlate(
      ...
      plateMat,
      foot,
      [toeSide * 0.07, -0.01, 0.26]
    );
    toe.rotation.x = -0.07;
    toe.rotation.y = toeSide * 0.06;
  });
...
function createPowerCreature(color, index) {
  const cfg = MORPHS.P;
  const root = new THREE.Group();
  const bodyMaster = new THREE.Group();
  bodyMaster.position.y = 2.04;
  root.add(bodyMaster);

  const baseColor = new THREE.Color(color);
  const primary = mat(baseColor);
  const secondary = mat(baseColor.clone().multiplyScalar(0.72));
  const joint = mat(baseColor.clone().multiplyScalar(0.43), 0.76);
  const underside = mat(0x24282c, 0.84);
  const plate = new THREE.MeshStandardMaterial({
    color: baseColor.clone().lerp(new THREE.Color(0x9fa8ad), 0.18),
    roughness: 0.64,
    metalness: 0.04,
    flatShading: true
  });
  const cue = new THREE.MeshStandardMaterial({
    color: 0x1a2026,
    emissive: baseColor.clone().multiplyScalar(0.44),
  ...
  const chestPivot = new THREE.Group();
  chestPivot.position.set(0, 0.02, 0.54);
  bodyMaster.add(chestPivot);

  const pelvisPivot = new THREE.Group();
  pelvisPivot.position.set(0, -0.02, -0.66);
  bodyMaster.add(pelvisPivot);

  // Same species construction as S, but with a deeper chest, larger rear mass,
  // and shorter longitudinal profile.
  makeMesh(
    new THREE.IcosahedronGeometry(0.70, 1),
    primary,
    chestPivot,
    [0, -0.02, 0.02],
    [0.98, 0.86, 1.12]
  ...
  const waist = makeMesh(
    new THREE.CylinderGeometry(0.34, 0.40, 0.78, 7),
    secondary,
    bodyMaster,
    [0, -0.10, -0.07]
  );
  ...
  const keel = makeMesh(
    new THREE.BoxGeometry(0.50, 0.16, 1.26),
    underside,
    bodyMaster,
    [0, -0.47, -0.03]
  );
  ...
  // P crest is broad / compact rather than the long S blade.
  const crestRoot = new THREE.Group();
  crestRoot.position.set(0, 0.23, 0.05);
  headPivot.add(crestRoot);
  ...
  const eyeMat = new THREE.MeshBasicMaterial({ color: 0xffd27a });
  [-1, 1].forEach((side) => {
    makeMesh(
      new THREE.SphereGeometry(0.032, 5, 4),
      eyeMat,
      headPivot,
      [side * 0.22, 0.055, 0.45]
    );
  });

  const cueBand = new THREE.Group();
  cueBand.position.set(0, 0.075, 0.22);
  headPivot.add(cueBand);

  const bandTop = makeMesh(
    new THREE.BoxGeometry(0.50, 0.065, 0.14),
    cue,
    cueBand,
    [0, 0.15, 0]
  );
  ...
  [-1, 1].forEach((side) => {
    makeMesh(
      new THREE.BoxGeometry(0.085, 0.19, 0.17),
      cue,
      cueBand,
      [side * 0.265, 0.015, 0.02]
    );
    makeMesh(
      new THREE.SphereGeometry(0.045, 6, 4),
      new THREE.MeshBasicMaterial({ color: side < 0 ? 0xffa63d : 0x6ee7ff }),
      cueBand,
      [side * 0.30, 0.015, 0.07]
    );
  });

  const legs = {
    fl: makePowerLimb(chestPivot, primary, secondary, joint, plate, -1, true),
    fr: makePowerLimb(chestPivot, primary, secondary, joint, plate, 1, true),
    hl: makePowerLimb(pelvisPivot, secondary, primary, joint, plate, -1, false),
    hr: makePowerLimb(pelvisPivot, secondary, primary, joint, plate, 1, false)
  };

  const tailBase = new THREE.Group();
  tailBase.position.set(0, 0.00, -0.65);
  tailBase.rotation.x = -0.28;
  pelvisPivot.add(tailBase);

  const tailSegments = [];
  let tailParent = tailBase;
  const tailLengths = [0.32, 0.28, 0.23];
  ...
  tailLengths.forEach((segLen, i) => {
    ...
    const seg = makeMesh(
      new THREE.CylinderGeometry(
        Math.max(0.055, 0.105 - i * 0.018),
        Math.max(0.070, 0.135 - i * 0.020),
        segLen,
        6
      ),
      i === 0 ? secondary : primary,
...
function createEndureCreature(color, index) {
  ...
  root.userData = {
    ...
    tailPitchState: tailSegments.map(() => 0),
    tailYawState: tailSegments.map(() => 0),
    strideLength: E_GAIT.minStrideWorld,
    maxStanceSlip: 0,
    minBodyY: E_GAIT.baseY,
    maxBodyY: E_GAIT.baseY
  };
...
function updateEndurePose(runner, lateralVelocity, dt) {
  const ud = runner.group.userData;
  const cfg = runner.cfg;
  const speedRatio = THREE.MathUtils.clamp(runner.speed / cfg.baseSpeed, 0, 1.16);
  const phase = ud.phase + runner.phaseBias;
  const cycle = wrap01(phase / TAU);
  const strideLength =
    ud.strideLength ||
    THREE.MathUtils.lerp(E_GAIT.minStrideWorld, E_GAIT.maxStrideWorld, speedRatio);

  const accelError = (runner.targetSpeed - runner.speed) / Math.max(cfg.baseSpeed, 1);
  ud.accelLean = THREE.MathUtils.damp(ud.accelLean ?? 0, accelError * 1.05, 8.0, dt);
  ud.turnLean = THREE.MathUtils.damp(
    ud.turnLean ?? 0,
    THREE.MathUtils.clamp(-lateralVelocity * cfg.laneLean * 0.14, -0.15, 0.15),
    9.0,
    dt
  );

  // E keeps energy in forward travel: low vertical excursion and an even load cycle.
  const rearLoad = Math.max(0, Math.sin((cycle - 0.02) * TAU));
  const foreLoad = Math.max(0, Math.sin((cycle - 0.50) * TAU));
  const load = Math.max(rearLoad, foreLoad);
  const suspension = Math.pow(Math.max(0, -Math.sin((cycle - 0.08) * TAU)), 1.55);
  const spineWave = Math.sin((cycle - 0.11) * TAU);

  const bodyY =
    E_GAIT.baseY +
    suspension * 0.062 * speedRatio -
    load * 0.032 * speedRatio;
  ud.bodyMaster.position.y = bodyY;

  const longStretch = spineWave * 0.090 * speedRatio;
  const compression = load * 0.038 * speedRatio;

  ud.chestPivot.position.z = E_GAIT.chestBaseZ + longStretch * 0.42;
  ud.pelvisPivot.position.z = E_GAIT.pelvisBaseZ - longStretch * 0.48;
  ud.chestPivot.position.y = E_GAIT.chestBaseY - compression * 0.54;
  ud.pelvisPivot.position.y = E_GAIT.pelvisBaseY - compression * 0.38;

  const bodyMidZ = (ud.chestPivot.position.z + ud.pelvisPivot.position.z) * 0.5;
  ud.waist.position.z = bodyMidZ;
  ud.waist.scale.y = 1 + Math.abs(longStretch) * 0.62;
  ud.keel.position.z = bodyMidZ;
  ud.keel.scale.z = 1 + Math.abs(longStretch) * 0.20;
  ...
  ud.chestPivot.rotation.x =
    -spineWave * 0.052 * speedRatio -
    foreLoad * 0.022 +
    suspension * 0.010;
  ud.pelvisPivot.rotation.x =
    spineWave * 0.064 * speedRatio +
    rearLoad * 0.026 -
    suspension * 0.010;
  ud.chestPivot.rotation.y = -Math.sin(phase * 0.5) * 0.009 * speedRatio;
  ud.pelvisPivot.rotation.y = Math.sin(phase * 0.5) * 0.012 * speedRatio;
  ...
  Object.values(ud.legs).forEach((leg) => {
    ...
    leg.hip.position.z =
      (leg.fore ? 0.30 : -0.28) +
      strideRoot * (leg.fore ? 0.060 : 0.072) * speedRatio;
    leg.hip.position.y =
      (leg.fore ? -0.10 : -0.09) +
      liftRoot * 0.018 * speedRatio -
      load * 0.008;
  });

  const stanceDuration = E_GAIT.stance;
  const stanceSweep = strideLength * stanceDuration;
  const halfSweep = stanceSweep * 0.5;

  Object.values(ud.legs).forEach((leg) => {
    const localCycle = wrap01((phase + leg.phaseOffset) / TAU);
    const nominalReach = leg.fore ? 1.48 : 1.50;
    ...
    if (localCycle < stanceDuration) {
      ...
      targetY =
        -nominalReach +
        compressionShape * 0.010 -
        load * 0.007;
      footPitch = THREE.MathUtils.lerp(-0.040, 0.095, u);
      ...
      // Smooth compact recovery with no violent fold / extension.
      const advance = 0.5 - 0.5 * Math.cos(u * Math.PI);
      const liftShape = Math.pow(Math.sin(u * Math.PI), 1.18);
      const settle = Math.pow(
        THREE.MathUtils.clamp((u - 0.72) / 0.28, 0, 1),
        1.5
      );
      ...
      targetY =
        -nominalReach +
        liftShape * E_GAIT.swingLift -
        settle * 0.018;
      footPitch =
        -0.145 * liftShape +
        THREE.MathUtils.lerp(0.045, -0.025, Math.min(1, u / 0.94));
    }
  ...
  ud.headPivot.rotation.x = ud.headLag;
  ud.headPivot.rotation.z = -ud.turnLean * 0.42;

  ud.tailSegments.forEach((joint, i) => {
    const pitchTarget =
      0.006 +
      ud.pelvisPivot.rotation.x * (0.13 + i * 0.04) -
      suspension * (0.006 + i * 0.002);
    const yawTarget =
      -ud.turnLean * (0.42 + i * 0.12) +
      Math.sin(phase * 0.30 - i * 0.44) * (0.007 + i * 0.003) * speedRatio;

    ud.tailPitchState[i] = THREE.MathUtils.damp(
      ud.tailPitchState[i] ?? pitchTarget,
      pitchTarget,
      Math.max(4.0, 8.2 - i * 1.0),
      dt
    );
    ud.tailYawState[i] = THREE.MathUtils.damp(
      ud.tailYawState[i] ?? yawTarget,
      yawTarget,
      Math.max(3.8, 7.8 - i * 0.9),
      dt
    );
  ...
  if (MOTION_REVIEW_MODE && REVIEW_MORPH === "E") {
    const legs = Object.values(ud.legs);
    canvas.dataset.eIkClamped = legs.some((leg) => leg.ikClamped) ? "1" : "0";
    canvas.dataset.eMaxStanceSlip = String(ud.maxStanceSlip || 0);

    ud.minBodyY = Math.min(ud.minBodyY ?? bodyY, bodyY);
    ud.maxBodyY = Math.max(ud.maxBodyY ?? bodyY, bodyY);
    canvas.dataset.eVerticalRange = String(ud.maxBodyY - ud.minBodyY);
  }
}
...
function updateCreaturePose(runner, lateralVelocity, dt = 1 / 60) {
  ...
  ud.tailSegments.forEach((joint, i) => {
    ...
    const powerTail = runner.morph === "P";
    joint.rotation.y =
      Math.sin(lag) * (powerTail ? 0.055 + i * 0.014 : 0.16 + i * 0.035) * speedRatio -
      ud.turnLean * (0.55 + i * 0.18);
    joint.rotation.x =
      (powerTail ? 0.035 : 0.18) +
      Math.sin(lag * 0.82 + 0.7) *
        (powerTail ? 0.022 + i * 0.008 : 0.06 + i * 0.022) *
        speedRatio;
  });
}
