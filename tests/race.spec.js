...
test("capture mobile race camera views", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("#stage")).toHaveAttribute("data-track-presentation", "v13", { timeout: 8000 });
...
test("record cinematic race presentation", async ({ browser }, testInfo) => {
  ...
  await expect(page.locator("#stage")).toHaveAttribute("data-presentation-speed", "1.65x", { timeout: 8000 });
