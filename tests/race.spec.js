...
test("capture mobile race camera views", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("#stage")).toHaveAttribute("data-track-presentation", "v17", { timeout: 8000 });
...
test("record cinematic race presentation", async ({ browser }, testInfo) => {
  ...
  await expect(page.locator("#stage")).toHaveAttribute("data-presentation-speed", "2.15x", { timeout: 8000 });
