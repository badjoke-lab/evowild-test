...
def nearest_anchor(obj,target_sx,target_h,target_u,head_sign):
    ...
    for v in obj.data.vertices:
        ...
        score=((sx-target_sx)/0.12)**2 + ((h-target_h)/0.10)**2 + ((u-target_u)/0.12)**2
        if score<best_score:
            best_score=score
            best=p.copy()
    return best
...
def collapse_old_crest_tail(obj):
    pts,b=mesh_bounds(obj)
    head_sign=detect_head_sign(pts,b)
    width=max(1e-6,b["max_x"]-b["min_x"])
    height=max(1e-6,b["max_y"]-b["min_y"])
    ...
    for v in obj.data.vertices:
        ...
        # Old crest: only the forward/high region. Collapse it into a compact
        # skull ridge around u=.44, leaving neck/shoulders untouched.
        crest=smoothstep(0.30,0.50,u)*smoothstep(0.68,0.84,h)
        if crest>0.0:
            target_z=cz+head_sign*length*0.44
            target_y=bottom+height*0.72
            x=lerp(x,cx,0.90*crest)
            y=lerp(y,target_y,0.82*crest)
            z=lerp(z,target_z,0.78*crest)

        # Old tail: restrict to the far-rear half to avoid hind-leg vertices.
        tail=smoothstep(0.42,0.62,-u)*smoothstep(0.38,0.72,h)
        if tail>0.0:
            target_z=cz-head_sign*length*0.39
            target_y=bottom+height*0.57
            x=lerp(x,cx,0.88*tail)
            y=lerp(y,target_y,0.84*tail)
            z=lerp(z,target_z,0.58*tail)
...
def add_new_crest_tail(body):
    ...
    # Crest roots are deliberately on the skull, not the shoulder/back.
    for side in (-1,1):
        a=nearest_anchor(body,side*0.10,0.72,0.49,head_sign)
        lateral=Vector((side*width,0,0))
        rear=Vector((0,0,-head_sign*length))
        up=Vector((0,height,0))
        pts2=[
            a,
            a+lateral*0.008+rear*0.035+up*0.055,
            a+lateral*0.014+rear*0.085+up*0.115,
            a+lateral*0.014+rear*0.145+up*0.170,
            a+lateral*0.008+rear*0.215+up*0.215,
        ]
        rs=[width*0.055,width*0.044,width*0.032,width*0.020,width*0.006]
        for i in range(4):
            o=cone_between(f"S_Crest52_{side}_{i}",pts2[i],pts2[i+1],rs[i],rs[i+1],10)
            if o: created.append(o)

    # Tail begins at the actual rump rear and stays nearly horizontal.
    a=nearest_anchor(body,0.0,0.58,-0.40,head_sign)
    rear=Vector((0,0,-head_sign*length))
    down=Vector((0,-height,0))
    pts2=[
        a,
        a+rear*0.075+down*0.004,
        a+rear*0.155+down*0.012,
        a+rear*0.235+down*0.025,
        a+rear*0.315+down*0.045,
    ]
    rs=[width*0.060,width*0.048,width*0.034,width*0.020,width*0.005]
    for i in range(4):
        o=cone_between(f"S_Tail52_{i}",pts2[i],pts2[i+1],rs[i],rs[i+1],8)
        if o: created.append(o)
...
def main():
    ...
    collapse_old_crest_tail(obj)
    obj,added_parts=add_new_crest_tail(obj)
    ...
    stats["status"]="shape_refine_v5_2_surface_anchored_candidate_not_canonical"
    ...
    print("SHAPE_V52",json.dumps(stats,separators=(",",":")))
