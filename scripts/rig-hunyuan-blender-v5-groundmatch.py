...
def animate_run_cycle(arm, dims):
    ...
    action = bpy.data.actions.new("EvoWild_S_Run_V5_GroundMatch")
    arm.animation_data_create()
    arm.animation_data.action = action

    frames = list(range(1, 20, 2))
    if frames[-1] != 25:
        frames.append(25)
    ...
    def contact_target(u, is_hind):
        ...
        if u < 0.40:
            ...
            t = (u - 0.50) / 0.28
            z = lerp(-0.50, 0.46 if is_hind else 0.42, t)
            y = lerp(0.055, 0.22, math.sin(min(1.0, t) * math.pi * 0.5))
            foot = lerp(0.22, -0.20, t)
        else:
            t = (u - 0.78) / 0.22
            z = lerp(0.46 if is_hind else 0.42, 0.42, t)
            y = lerp(0.22, 0.0, t)
...
def main():
    ...
    metadata = {
        ...
        "status": "ground_matched_ik_rig_v5_proof_not_final_art"
    }
    ...
    print("RIG_V5", json.dumps(metadata, separators=(",", ":")))
