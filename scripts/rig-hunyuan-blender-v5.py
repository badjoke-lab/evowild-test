...
def animate_run_cycle(arm, dims):
    ...
    action = bpy.data.actions.new("EvoWild_S_Run_V5")
    ...
    for frame in frames:
        ...
        for limb, offset in limb_phase.items():
            ...
            recovery = max(0.0, math.sin(phase + 0.62))
            stance = max(0.0, -math.sin(phase + 0.16))

            upper_amp = 0.66 if is_hind else 0.58
            upper_bias = 0.035 if is_hind else -0.02
            upper_x = upper_bias - swing * upper_amp

            lower_base = 0.16 if is_hind else 0.14
            lower_recovery = 0.56 if is_hind else 0.62
            lower_x = lower_base + lower_recovery * recovery - 0.055 * stance

            # Counter-rotate the foot against the upstream chain. This is not
            # full IK, but it keeps the contact phase visibly flatter.
            foot_x = -(upper_x * 0.34 + lower_x * 0.62) + 0.16 * stance
            lateral = side * (0.012 + 0.008 * math.sin(phase + 0.4))

            set_rot(f"{limb}_upper", frame, upper_x, 0.0, lateral)
            set_rot(f"{limb}_lower", frame, lower_x, 0.0, lateral * 0.30)
            set_rot(f"{limb}_foot", frame, foot_x, 0.0, -lateral * 0.22)

        pelvis = 0.040 * math.sin(base - 0.28)
        spine = 0.038 * math.sin(base + 0.18)
        chest = -0.030 * math.sin(base + 0.38)
        neck = -0.035 * math.sin(base + 0.08)
        head = -0.020 * math.sin(base - 0.04)
        tail = 0.080 * math.sin(base + math.pi * 0.65)
        ...
        suspension = max(0.0, math.sin(base - 0.45))
        compression = math.cos(base)
        bob = dims["height"] * 0.018
        arm.location.y = bob * (0.48 * suspension - 0.26 * compression)
        arm.keyframe_insert(data_path="location", frame=frame, index=1)

        surge = dims["length"] * 0.0028 * math.sin(base - 0.75)
...
def main():
    ...
    metadata = {
        ...
        "status": "stance_direction_corrected_v5_proof_not_final_art"
    }
    ...
    print("RIG_V3", json.dumps(metadata, separators=(",", ":")))
