...
def make_armature(b, head_sign):
    ...
    arm.name = "EvoWild_S_Armature_V3"
...
def animate_run_cycle(arm, dims):
    ...
    action = bpy.data.actions.new("EvoWild_S_Run_V3_1")
    ...
    # The offsets deliberately avoid perfect diagonal symmetry so a pack of
    # racers does not read as a synchronized mechanical trot.
    limb_phase = {
        "hind_L": 0.00,
        "hind_R": 0.43,
        "fore_L": 0.54,
    ...
    def limb_pose(u, is_hind):
        # 0.00-0.38 stance/contact: foot stays visually flatter while the
        # body moves over it. 0.38-0.50 push-off extends the chain.
        # 0.50-0.78 recovery folds sharply for clearance.
        # 0.78-1.00 landing reaches forward and re-levels the foot.
        if u < 0.38:
            t = u / 0.38
            upper = lerp(0.34, -0.28, t)
            lower = lerp(0.22, 0.10, t)
            foot = lerp(-0.12, -0.06, t)
            lift = 0.0
        elif u < 0.50:
            t = (u - 0.38) / 0.12
            upper = lerp(-0.28, -0.42, t)
            lower = lerp(0.10, 0.02, t)
            foot = lerp(-0.06, 0.18, t)
            lift = lerp(0.0, 1.0, t)
        elif u < 0.78:
            t = (u - 0.50) / 0.28
            upper = lerp(-0.42, 0.36 if is_hind else 0.30, t)
            lower = lerp(0.02, 0.68 if is_hind else 0.74, t)
            foot = lerp(0.18, -0.20, t)
            lift = 1.0
        else:
            t = (u - 0.78) / 0.22
            upper = lerp(0.36 if is_hind else 0.30, 0.34, t)
            lower = lerp(0.68 if is_hind else 0.74, 0.22, t)
            foot = lerp(-0.20, -0.12, t)
            lift = lerp(1.0, 0.0, t)

        # Hind limbs contribute more drive, forelimbs more reach.
        if is_hind:
            upper *= 1.08
            lower *= 0.94
        else:
    ...
    for frame in frames:
        ...
        for limb, offset in limb_phase.items():
            ...
            upper_x, lower_x, foot_x, lift = limb_pose(u, is_hind)
            lift_values.append(lift)

            # Lateral motion remains deliberately small; the contact phase
            # should read from sagittal motion rather than leg splay.
            lateral = side * (0.010 + 0.006 * math.sin((u * math.tau) + 0.35))

            set_rot(f"{limb}_upper", frame, upper_x, 0.0, lateral)
            set_rot(f"{limb}_lower", frame, lower_x, 0.0, lateral * 0.25)
            set_rot(f"{limb}_foot", frame, foot_x, 0.0, -lateral * 0.18)

        # Torso follows the aggregate suspension state rather than a plain
        # sine wave, so the body stays steadier while one or more feet plant.
        airborne = sum(lift_values) / len(lift_values)
        pelvis = 0.028 * math.sin(base - 0.24)
        spine = 0.030 * math.sin(base + 0.14)
        chest = -0.024 * math.sin(base + 0.34)
        neck = -0.030 * math.sin(base + 0.08)
        head = -0.016 * math.sin(base - 0.02)
        tail = 0.070 * math.sin(base + math.pi * 0.62)

        set_rot("pelvis", frame, pelvis)
        set_rot("spine", frame, spine)
        set_rot("chest", frame, chest)
        set_rot("neck", frame, neck)
        set_rot("head", frame, head)
        set_rot("tail", frame, tail)

        bob = dims["height"] * 0.015
        arm.location.y = bob * (airborne - 0.42)
        arm.keyframe_insert(data_path="location", frame=frame, index=1)

        surge = dims["length"] * 0.0022 * math.sin(base - 0.70)
...
def main():
    ...
    print("RIG_V31", json.dumps(metadata, separators=(",", ":")))
