...
def normalized_shape_coords(p,b,head_sign):
    height=max(1e-6,b["max_y"]-b["min_y"])
    length=max(1e-6,b["max_z"]-b["min_z"])
    width=max(1e-6,b["max_x"]-b["min_x"])
    cx=(b["min_x"]+b["max_x"])*0.5
...
def nearest_surface_anchor(obj,target_sx,target_h,target_u,head_sign):
    pts,b=mesh_bounds(obj)
    best=None
    best_score=1e9
    for v in obj.data.vertices:
        p=obj.matrix_world @ v.co
        sx,h,u=normalized_shape_coords(p,b,head_sign)
        score=((sx-target_sx)/0.16)**2 + ((h-target_h)/0.14)**2 + ((u-target_u)/0.16)**2
        if score<best_score:
            best_score=score
            best=p.copy()
    return best
...
def collapse_legacy_appendages(obj):
    ...
    crest_base_y=bottom+height*0.71
    crest_base_u=0.30
    tail_axis_y=bottom+height*0.53
    ...
    for v in obj.data.vertices:
        ...
        # Strongly collapse the old many-pronged crest into a skull-hugging
        # base that the new two-blade crest can grow from.
        crest=smoothstep(0.10,0.34,u)*smoothstep(0.64,0.82,h)
        if crest>0.0:
            target_z=cz + head_sign*length*crest_base_u
            x=lerp(x,cx,0.88*crest)
            y=lerp(y,crest_base_y,0.78*crest)
            z=lerp(z,target_z,0.72*crest)

        # Collapse the old feather fan toward a narrow rear axis.
        tail=smoothstep(0.26,0.52,-u)*smoothstep(0.32,0.70,h)
        if tail>0.0:
            axis_y=tail_axis_y-height*0.055*smoothstep(0.26,0.56,-u)
            x=lerp(x,cx,0.84*tail)
            y=lerp(y,axis_y,0.82*tail)
...
def cone_between(name,a,b,r1,r2,vertices=10):
...
def add_sprint_appendages(body):
    ...
    # Anchor each crest blade directly on the post-collapse skull surface.
    for side in (-1,1):
        anchor=nearest_surface_anchor(body, side*0.12, 0.71, 0.30, head_sign)
        side_vec=Vector((side*width,0,0))
        forward=Vector((0,0,head_sign*length))
        up=Vector((0,height,0))
        points=[
            anchor,
            anchor + side_vec*0.012 + up*0.075 - forward*0.045,
            anchor + side_vec*0.018 + up*0.145 - forward*0.105,
            anchor + side_vec*0.016 + up*0.205 - forward*0.175,
            anchor + side_vec*0.010 + up*0.245 - forward*0.235,
        ]
        radii=[width*0.055,width*0.043,width*0.032,width*0.020,width*0.006]
        for i in range(len(points)-1):
            o=cone_between(
                f"S_Crest51_{'L' if side>0 else 'R'}_{i}",
                points[i],points[i+1],radii[i],radii[i+1],vertices=10
            )
            if o: created.append(o)

    # Tail also begins on the actual rump surface rather than a guessed point.
    anchor=nearest_surface_anchor(body,0.0,0.54,-0.30,head_sign)
    rear=Vector((0,0,-head_sign*length))
    down=Vector((0,-height,0))
    points=[
        anchor,
        anchor + rear*0.10 + down*0.012,
        anchor + rear*0.20 + down*0.035,
        anchor + rear*0.30 + down*0.065,
        anchor + rear*0.39 + down*0.095,
    ]
    radii=[width*0.060,width*0.048,width*0.035,width*0.021,width*0.006]
    for i in range(len(points)-1):
        o=cone_between(f"S_Tail51_{i}",points[i],points[i+1],radii[i],radii[i+1],vertices=8)
        if o: created.append(o)
...
def main():
    ...
    stats["status"]="shape_refine_v5_1_surface_anchored_candidate_not_canonical"
    ...
    print("SHAPE_V51",json.dumps(stats,separators=(",",":")))
