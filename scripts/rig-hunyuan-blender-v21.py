...
def animate_run_cycle(arm, dims):
    ...
    action = bpy.data.actions.new("EvoWild_S_Run_V2_1")
    ...
    for frame in frames:
        ...
        for limb, offset in limb_phase.items():
            ...
            upper_amp = 0.54 if is_hind else 0.48
            upper_bias = 0.04 if is_hind else -0.025
            ...
            if is_hind:
                lower_x = 0.18 + 0.62 * recovery - 0.08 * plant
            else:
                lower_x = 0.16 + 0.70 * recovery - 0.06 * plant
            ...
            lateral = side * (0.014 + 0.010 * math.sin(phase + 0.4))
        ...
        body_pitch = -0.040 + 0.050 * math.sin(base_phase - 0.35)
        spine_flex = 0.060 * math.sin(base_phase + 0.25)
        neck_counter = -0.045 * math.sin(base_phase + 0.10)
        head_counter = -0.024 * math.sin(base_phase - 0.05)
        tail_flex = 0.095 * math.sin(base_phase + math.pi * 0.65)
        ...
        surge = dims["length"] * 0.0035 * math.sin(base_phase - 0.75)
